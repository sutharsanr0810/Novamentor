...
def get_configured_api_keys() -> str:
    """Retrieves API keys from secrets or environment."""
    try:
...
class BaseMentorAgent:
    ...
    def fallback_mock_response(self, prompt: str, context: str) -> str:
        clean_prompt = prompt[:90].replace('"', "'").replace('\n', ' ')
        return (
            f'**[{self.name} Analysis]**\n\n'
            f'**1. Core Assessment:**\n'
            f'Evaluating: *{clean_prompt}...* against project parameters.\n\n'
            f'**2. Key Technical Directives:**\n'
            f'- Quantize inference tensors to INT8 to sustain high frame rates.\n'
            f'- Enforce DMA buffers to prevent memory saturation during frame ingestion.\n'
            f'- Validate thermal gradient filtering to reject non-target reflections.\n\n'
            f'**3. Defense Checkpoint:**\n'
            f'Ensure evaluation metrics compare mAP and latency against standard baselines.'
        )
...
class ArchitectureAgent(BaseMentorAgent):
    ...
    def fallback_mock_response(self, prompt: str, context: str) -> str:
        mermaid_block = (
            '```mermaid\n'
            'flowchart TD\n'
            '    A[FLIR Lepton 3.5 Sensor] -->|Raw 16-bit Stream| B[RPi 5 Host DMA Buffer]\n'
            '    B -->|Preprocessed Tensor| C[Hailo-8L NPU Accelerator]\n'
            '    C -->|Hotspot Scores| D{Threshold Evaluator}\n'
        ...
        return (
            ...
            '- Offloading inference to Hailo-8L frees CPU cores for path planning and MAVLink telemetry.'
        )
